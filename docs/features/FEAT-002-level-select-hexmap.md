...
### 2026-09-12 · 开场判据修订（回归修复）

- 本 FEAT 当时把开场演出的判据从「盘序还在 0」改成「地图上一片空白」（`_cleared_stages.is_empty() and _resume_stage_id == ""`），
  理由是盘序在关卡之间会重置、单看它会每次回标题都重放。副作用没被察觉：**只要通关过任何一关，耳机提示与开场剧情就永久消失**，
  老玩家的存档再也看不到开场。用户 2026-09-12 反馈「加回开幕剧情和耳机提示」。
- 改法不是退回老判据（那条确实会重放），而是把「放过没有」显式落盘：新增存档键 `intro_played`（照 `music_break_played` 的先例）。
  没放过就放 → 放完 `_intro_played = true` 并 `_save_progress()`；老档缺这个键、读出来是 false，因此有进度的存档会补放一次。
  「放弃本轮」写盘时保留该键；清空存档走 `GameSave.clear()` + 回标题重读，标记自然回到未放过。
- 回归测试 `tests/test_opening_intro_gate.gd` 专门用一份「打通了第一关且没有 intro_played」的独立存档，
  验补放 → 落盘 → 第二次不重放。`tests/test_stage_run_flow.gd` 里用「有推图战绩」跳过开场的那处改成了设 `_intro_played`。
